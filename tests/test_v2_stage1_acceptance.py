...
def _fixture(tmp_path: Path):
    ...
    stage0 = _write_json(
        ...
        {"accepted": True, "counters": {"errors": 0}, "artifacts": {"dataset_sha256": "d" * 64}},
    )
...
def test_accepts_current_schema_and_writes_marker(tmp_path):
...
def test_rejects_stage0_hash_and_missing_128_hand_stress(tmp_path):
...
def test_rejects_archive_source_drift_and_removes_stale_marker(tmp_path):
    root, stage0, build, parity, archive, _ = _fixture(tmp_path)
    marker = root / "checkpoints/rl_v2_stage1/STAGE1_ACCEPTED.json"
    write_stage1_marker(stage0, build, parity, marker)
    assert marker.exists()
